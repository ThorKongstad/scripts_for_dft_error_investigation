...
def main(neb_image_1: str, slab_for_1: str, neb_image_2: str, slab_for_2: str):
    ...
    energy_diff = neb_2_pd.iloc[0, 'energy'].values[0] + slab_1_pd.iloc[0, 'energy'].values[0] - neb_1_pd.iloc[0, 'energy'].values[0] + slab_2_pd.iloc[0, 'energy'].values[0]

    neb_1_ens = bytes_to_object(neb_1_pd.iloc[0, '_data'].values[0]).get('ensemble_en')
    slab_1_ens = bytes_to_object(slab_1_pd.iloc[0, '_data'].values[0]).get('ensemble_en')
    neb_2_ens = bytes_to_object(neb_2_pd.iloc[0, '_data'].values[0]).get('ensemble_en')
    slab_2_ens = bytes_to_object(slab_2_pd.iloc[0, '_data'].values[0]).get('ensemble_en')

    energy_ensemble = [n_2 + s_2 - n_1 - s_1 for n_1, s_1, n_2, s_2 in zip(neb_1_ens, slab_1_ens, neb_2_ens, slab_2_ens)]

    print(f'energy difference between image 1 and the reference 2 is: {energy_diff:.3f} +- {sd(energy_ensemble, mean_value=mean(energy_ensemble)):.3f}')
