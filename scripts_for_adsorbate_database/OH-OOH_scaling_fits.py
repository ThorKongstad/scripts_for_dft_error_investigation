            ...
            try:
                ...
                for i, fit in enumerate(fit_ens_objs):
                    ...
                    for oh_row, ooh_row in zip(oh_ensamble,ooh_ensamble):
                        oh_ensamble.extend(oh_row)
                        ooh_ensamble.extend(ooh_row)
        ...
        updatemenus=[
            dict(
                ...
                buttons=[
                    ...
                    dict(
                        args=[{"visible": [False if match(f'BEE for [A-Z][a-z] BEEF-vdW', trace.name) else 'undefined' for i, trace in enumerate(fig.data)],
                               'error_x.visible': [True if match('BEEF-vdW-[A-Z][a-z]', trace.name) else 'undefined' for i, trace in enumerate(fig.data)],
                               'error_y.visible': [True if match('BEEF-vdW-[A-Z][a-z]', trace.name) else 'undefined' for i, trace in enumerate(fig.data)],
                               }],
                        label='Error bars',
                        method='restyle',
                    ),
