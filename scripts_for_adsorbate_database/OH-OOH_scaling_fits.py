    ...
    for xc in functional_list:
        ...
        try:
            ...
            fig.add_trace(go.Scatter(mode='line',
                                     ...
                                     hoverinfo=f'XC: {xc.name}'+'<br>'+f'Slope: {fit_obj.slope:.3f} +- {fit_obj.stderr:.3f}'+'<br>'+f'Intercept: {fit_obj.intercept:.3f} +- {fit_obj.intercept_stderr:.3f}'+'<br>'+f'R-square: {fit_obj.rvalue:.3f}',
                                     **line_arg
                                     ))
            ...
            try:
                ...
                for i, fit in enumerate(fit_ens_objs):
                    fig.add_trace(go.Scatter(mode='line',
                                             ...
                                             hoverinfo=f'XC: BEE No. {i} for {xc.name}'+'<br>'+f'Slope: {fit.slope:.3f} +- {fit.stderr:.3f}'+'<br>'+f'Intercept: {fit.intercept:.3f} +- {fit.intercept_stderr:.3f}'+'<br>'+f'R-square: {fit.rvalue:.3f}',
