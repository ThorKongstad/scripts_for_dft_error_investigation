...
def scaling_plot(functional_list: Sequence[Functional], oh_reactions: Sequence[adsorbate_reaction], o_reactions: Sequence[adsorbate_reaction], png_bool: bool = False):
    ...
    for xc in functional_list:
        ...
        try:
            fit_obj = stats.linregress(x=(oh_adsor := list(map(xc.calculate_reaction_enthalpy, oh_reactions))),
                                       y=(o_adsor := list(map(xc.calculate_reaction_enthalpy, o_reactions))), )
            ...
            O_adsorption_values.extend(o_adsor)
        ...
        if xc.has_BEE:
            try:
                ...
                ooh_ensamble = list(map(xc.calculate_BEE_reaction_enthalpy, o_reactions))
    ...
    Concatenated_fit = stats.linregress(x=OH_adsorption_values, y=O_adsorption_values)
    fig.add_trace(go.Scatter(
        mode='lines',
        x=list(line),
    ...
    for oh_reac, o_reac in zip(oh_reactions, o_reactions):
        ...
        for xc in functional_list:
            ...
            try: fig.add_trace(go.Scatter(
                ...
                hovertemplate=f'metal: {metal}' + '<br>' + f'XC: {xc.name}' + '<br>' + f'OH adsorption: {str(oh_reac)}' + '   %{x:.3f}' + '<br>' + f'O adsorption: {str(o_reac)}' + '   %{y:.3f}',
                legendgroup=metal,
            ...
            if xc.has_BEE:
                try:
                    fig.add_trace(go.Scatter(
                        ...
                        hovertemplate=f'metal: {metal}' + '<br>' + f'OH adsorption: {str(oh_reac)}' + '   %{x:.3f}' + '<br>' + f'O adsorption: {str(o_reac)}' + '   %{y:.3f}',
    ...
    save_name = 'reaction_plots/scaling_plot_OH_O_fits'
