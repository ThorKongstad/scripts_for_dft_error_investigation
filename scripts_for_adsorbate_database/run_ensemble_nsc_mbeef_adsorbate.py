...
def main(db_id: int, db_dir: str = 'molreact.db'):
    ...
    calc = GPAW(mode=PW(500),
                ...
                kpts=[4, 4, 1],
                basis='dzp',
                txt=f'{functional_folder}/mBEEF_ensambel_{structure_str}_{db_id}.txt',
    ...
    mBEEF_ensem_en_li = ens.get_ensemble_energies()

    if world.rank == 0:
        data_dict.update({'mBEEF_energy': mBEEF_potential_e, 'mBEEF_ensemble_en': mBEEF_ensem_en_li})
        update_db(db_dir, dict(id=db_id, mBEEF_ensemble_bool=True, data=data_dict))
