...
def overpotential(dG_OOH: float, dG_OH: float, dG_O: float) -> float: return 1.23 - min((4.92 - dG_OOH, dG_OH)) # dG_OOH - dG_O, dG_O - dG_OH
    ...
    for xc in functional_list:
        ...
        for oh_reac, ooh_reac in zip(oh_reactions, ooh_reactions):
            try: fig.add_trace(go.Scatter(
                ...
                y=[overpotential(
                    dG_OOH=(ooh_adsorp := xc.calculate_reaction_energy(ooh_reac)),
                    dG_OH=oh_adsorp,
                    dG_O=oh_adsorp*2
                )],
