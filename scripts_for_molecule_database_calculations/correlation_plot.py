...
def BEE_reaction_enthalpy(reac:reaction, functional: str, dbo: db.core.Database | pd.DataFrame) -> np.ndarray:
    if isinstance(dbo, db.core.Database):
        ...
        reac_ensamble_enthalpy = np.sum((np.array(
            ...
            + (np.array(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('enthalpy').iloc[0])
            - np.array(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('energy').iloc[0])) * amount
                                      for smile, amount in reac.reactants), axis=0)
        prod_ensamble_enthalpy = np.sum((np.array(
            bytes_to_object(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('_data').iloc[0]).get('ensemble_en')[:]) + (
                        np.array(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('enthalpy').iloc[0])
                        - np.array(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('energy').iloc[0])) * amount for smile, amount in reac.products), axis=0)
        return reac_ensamble_enthalpy - prod_ensamble_enthalpy
    raise ValueError('The type of database object was not recognised')
