...
def reaction_enthalpy(reac: reaction, functional: str, dbo: db.core.Database | pd.DataFrame = None, bee: bool = False) -> float | Tuple[float, float]:
    ...
    if isinstance(dbo, db.core.Database):
        ...
        reac_enthalpy = sum(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('enthalpy')*amount for smile,amount in reac.reactants)
        prod_enthalpy = sum(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('enthalpy')*amount for smile,amount in reac.products)
        ...
        if bee and functional in ('BEEF-vdW',):
            reac_ensamble_enthalpy = np.sum((np.array(bytes_to_object(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('_data').iloc[0]).get('ensemble_en')[:]
                                                      +(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('enthalpy')
                                                        -dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('energy')))*amount for smile,amount in reac.reactants),axis=0)
            prod_ensamble_enthalpy = np.sum((np.array(bytes_to_object(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('_data').iloc[0]).get('ensemble_en')[:]
                                                      +(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('enthalpy')
                                                      - dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('energy')))*amount for smile,amount in reac.products),axis=0)
            error_dev = (prod_ensamble_enthalpy-reac_ensamble_enthalpy).std()
            return error_dev, prod_enthalpy - reac_enthalpy
        return prod_enthalpy - reac_enthalpy
...
def BEE_reaction_enthalpy(reac:reaction, functional:str, dbo: Optional[db.core.Database | pd.DataFrame] = None) -> np.ndarray:
    ...
    if isinstance(dbo, db.core.Database):
        ...
        reac_ensamble_enthalpy = np.sum((np.array(
            ...
            + (dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('enthalpy')
            - dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('energy'))) * amount
...
def BEE_reaction_enthalpy_final_energy_correction(reac:reaction, functional:str, dbo: Optional[db.core.Database | pd.DataFrame] = None) -> np.ndarray:
    ...
    if isinstance(dbo, db.core.Database):
        ...
        correction = reaction_enthalpy(reac, functional, dbo) \
                     -(sum(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('energy')*amount for smile,amount in reac.reactants)
                     -sum(dbo.query(f'smiles == "{smile}" and xc == "{functional}" and enthalpy.notna()').get('energy')*amount for smile,amount in reac.products))
        reac_ensamble_enthalpy = sum(np.array(
